...
def hashlength_bits(value: str, bit_length: int) -> np.ndarray:
    ...
    if bit_length == 32:
        ...
        raw = h.to_bytes(4, byteorder="big", signed=False)
    elif bit_length == 64:
        h128 = mmh3.hash128(value, seed=0, signed=False)
        h = h128 & ((1 << 64) - 1)
        raw = h.to_bytes(8, byteorder="big", signed=False)
    elif bit_length == 128:
        h = mmh3.hash128(value, seed=0, signed=False)
        raw = h.to_bytes(16, byteorder="big", signed=False)
    elif bit_length == 256:
        h1 = mmh3.hash128(value, seed=0, signed=False)
        h2 = mmh3.hash128(value, seed=1, signed=False)
        raw = h1.to_bytes(16, byteorder="big", signed=False) + h2.to_bytes(16, byteorder="big", signed=False)
    else:
        raise ValueError("bit_length 仅支持 32/64/128/256")

    return np.unpackbits(np.frombuffer(raw, dtype=np.uint8)).astype(np.int64)
...
def derive_aux_basis(key_t: str, bit_length: int, p: int) -> np.ndarray:
    if bit_length == 32:
        return np.array(
            [
                (mmh3.hash(f"{key_t}:{i}", signed=False) % (p - 1)) + 1
